...
- **Two-stage candidate pair cap** (`config.py`, `designer/design.py`, `pipeline.py`): Replaced the single `max_pairs_per_junction` cap with a two-stage approach. A loose `max_pairs_pre_filter` cap (default 500) is applied immediately after pair design to limit memory and BLAST workload. The tighter `max_pairs_per_junction` cap (default 100) is now applied after SNP and BLAST filtering, right before multiplex optimization. This prevents constrained junctions from losing viable pairs to an early cap that fires before quality filters have had a chance to run. Both parameters can be set to `null` to disable.
