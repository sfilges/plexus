...
- **Candidate pair cap per junction** (`config.py`, `designer/design.py`): New `max_pairs_per_junction` parameter (default 100) in `primer_pair_parameters`. After pair formation, candidate pairs are ranked by `pair_penalty` and truncated to the top N. Bounds the DFS search space for junctions with thousands of candidates without meaningful quality loss. Set to `null` to disable.
